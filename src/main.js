...
const features = [
  ...
  { index: "02", title: "Guard the boundary", body: "QuotaFence evaluates policy before a managed agent launch and can warn or refuse work at configured thresholds." },
...
app.innerHTML = `
  ...
  <main id="top">
    ...
    <section class="workflow-section" id="workflow"><div class="section-shell workflow-grid">
      <div class="workflow-copy reveal"><p class="section-label">How it works</p><h2>One command.<br>Four quiet checks.</h2><p>Run a supported agent from an allocated folder. QuotaFence resolves context, checks policy, supervises the process, and reconciles usage when the session ends.</p><div class="terminal"><div class="terminal-bar"><span></span><span></span><span></span><small>Codex example · ~/Code/quotafence</small></div><code><span>❯</span> qfence codex</code><p><i>QuotaFence:</i> launching Codex for quotafence with 2 percentage points reserved (Weekly)</p><div class="terminal-alternative"><span>Claude works the same way</span><code>qfence claude</code></div></div></div>
